//
//  MPScrollViewViewController.m
//  MobileProject
//
//  Created by wujunyang on 2017/12/9.
...
@interface MPScrollViewViewController()<UITableViewDataSource, UITableViewDelegate>
@property (nonatomic,strong) NSArray             *dataArray;
@property (nonatomic,strong) UITableView         *myTableView;
@property(nonatomic,strong)UIScrollView *vyScrollView;
@property(nonatomic,strong)UIView *vyScrollViewContentView;

@end

@implementation MPScrollViewViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self.view addSubview:self.vyScrollView];
    [self.vyScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.left.and.right.mas_equalTo(0);
        make.bottom.mas_equalTo(-50);
    }];
    
    [self.vyScrollView addSubview:self.vyScrollViewContentView];
    [self.vyScrollViewContentView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.vyScrollView);
        make.width.equalTo(self.vyScrollView);
    }];
    ...
    if (!_myTableView) {
        ...
        [self.vyScrollViewContentView addSubview:_myTableView];
    ...
    [self.vyScrollViewContentView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(self.myTableView);
    }];
}
...
-(UIScrollView*)vyScrollView
{
    if (!_vyScrollView)
    {
        _vyScrollView=[[UIScrollView alloc]init];
        _vyScrollView.backgroundColor=[UIColor redColor];
    }
    return _vyScrollView;
}

-(UIView *)vyScrollViewContentView
{
    if (!_vyScrollViewContentView) {
        _vyScrollViewContentView=[[UIView alloc]init];
        _vyScrollViewContentView.backgroundColor=[UIColor blueColor];
    }
    return _vyScrollViewContentView;
}
